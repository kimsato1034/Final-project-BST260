---
title: "Analsys of Boston's Bluebike Data"
...
output_tree <- ctree(overtime ~ gender + factor(rain_cat) + factor(snownice_cat),
                     data = bbikemember)
plot(output_tree)
```
...
In this logistic regression model, we put gender, age, weekend, rain, snow, the popularity of starting stations, and the popularity of ending stations in order to predict overtime riders. We created training dataset for building a model and testing dataset for check the performance of the model. Because of the big sample size, we got statistically higher or lower odds of over-riding in all of the covariates. However, if we look at the odds ratio, we could realize something interesting. The odds ratio of over-riding when it snowed is 0.54 (95% CI: 0.50 to 0.59) times the odds when it did not. The odds ratio of over-riding when it rained is 0.92 (95% CI: 0.91 to 0.93) times the odds when it did not. If it rains or snows, people tend to return bike on time. On the other hand, the odds ratio of over-riding on weekend is 1.28 (95% CI: 1.26 to 1.30) times the odds on weekday. The odds ratio of over-riding among females is 1.37 (95% CI: 1.35 to 1.39) times the odds among males. These data shows females tend to use Blue Bike longer than 45 minutes especially on weekend.
Finally we drew the ROC curve using the testing dataset. AUC is 0.61, which means this model is not so good model to predict over-ride.
